      ...
      <nav className="flex justify-between items-center md:px-10 py-5 px-4 border-b border-green-400/10 sticky top-0 dark:bg-[#080c0a]/90 backdrop-blur-md z-50 bg-card">
        ...
        <div className="flex md:hidden items-center gap-3">
          ...
          <button
            ...
            className="w-8 h-8 flex items-center justify-center rounded-lg border border-green-400/30 text-green-400"
            aria-label="Toggle menu"
          >
            <i className={`ti ${menuOpen ? 'ti-x' : 'ti-menu-2'} text-base`} />
          </button>
        </div>
      </nav>

{menuOpen && (
        <div className="md:hidden fixed top-[57px] left-0 right-0 z-40 dark:bg-[#080c0a] bg-card border-b border-green-400/10 flex flex-col px-6 py-4 gap-4">
          <a
            href="#features"
          ...
          <Link
            href="/Login"
            onClick={() => setMenuOpen(false)}
            className="bg-green-400 text-black font-mono font-bold text-sm px-5 py-2.5 rounded hover:opacity-80 transition-opacity text-center mt-1"
          >
            Get Started
          </Link>
        </div>
