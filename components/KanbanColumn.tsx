  ...
  return (
    <div
      ...
      className={`flex-1 rounded-lg p-4 border transition-colors ${
        isOver ? 'border-green-400 bg-green-400/5' : 'border-zinc-700 bg-card'
      }`}
    >
      {/* Column Header */}
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="ml-auto dark:text-gray-400 text-zinc-700 text-sm">{bugs.length}</span>
      </div>

      {/* Bug Cards */}
      <div className="flex flex-col gap-3 min-h-24 ">
        {bugs.length === 0 && (
          <p className="dark:text-gray-600 text-gray-800 text-sm text-center mt-6">No bugs here</p>
        )}
